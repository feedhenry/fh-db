...
var _importZip = function(filePath, cb, retries){
    ...
    for (var z = 0; z < zipEntries.length; z++){
      if (zipEntries[z].isDirectory && zipEntries[z].entryName.indexOf('__MACOSX') > -1) {
        zip.deleteFile(zipEntries[z]);
      }
    }
    ...
    for (var i = 0; i < zipEntries.length; i++){
      ...
      parsers[name] = function(parserData) {
        return function(parserCallback){
          importHandler[fileType](parserData, parserCallback);
        };
      }(data);
    }
